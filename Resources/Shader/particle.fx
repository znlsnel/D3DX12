...
{
        float3  worldPos; // 위치
        float   curTime; // 생성이후 경과시간
        float3  worldDir; // 어느 방향으로 갈지
        float   lifeTime; // 생명주기
        int     alive; // 살아있는지 죽었는지
        float3  padding; // 16바이트 맞추기 위한 padding
};
...
void CS_Main(int3 threadIndex : SV_DispatchThreadID)
{
        ...
        GroupMemoryBarrierWithGroupSync(); // 중복 방지 멈춰
        ...
        if (g_particle[threadIndex.x].alive == 0)
        {
                while (true)
                {
                        ...
                        // interlock - 스레드 하나씩만 사용할 수 있음( 동시 접근 방지 )
